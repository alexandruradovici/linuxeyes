...
void resettime()
{
    ...
    setcolor (3,1);
    for (unsigned int j=0;j<=nrcols+1;j++) mvprintw (0,j," ");
    if (init_text == 1) mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v0.6 [press h for help]");
    else 
    {
...
int loadconfig()
{
  ...
  if (!f.fail())
  {
     ...
     f >>current_timeformat >>status_bar;
  }
  else r=0;
  f.close();
...
/*    switch (current_timeformat)
    ...
    setcolor (1,0);
    if (message.code==XA_MSG_NOTIFY_INPUT_TIMECODE) mvprintw (dx,nrcols-6,"[%.2d:%.2d]",message.data.timecode.m,message.data.timecode.s);
...
int main(int argc,char *argv[])
{
    ...
    add_console_message ("Hello, welcome to LinuxEyes v0.6");
    add_console_message ("--------------------------------");
    strcpy (strtmp,"xmp: starting in: \"");
    strcat (strtmp,path);
    strcat (strtmp,"\"");
    add_console_message (strtmp);
    printf ("Linux Eyes v0.6 (C) Alexandru Radovici 2002 - 2100\n\r");
...
//    keypad (NULL,TRUE);
    ...
    setcolor (3,1);
    init_text=1;
    mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v0.6 [press h for help]");
