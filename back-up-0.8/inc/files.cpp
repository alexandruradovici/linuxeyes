...
void showfiles(int scr,char *file,char s[100],int typeofshow, char *dir)
{
  int q=0;
  int v;
  ...
  do
  {
	...
	if (S_ISDIR(BUF.st_mode) != 0) mvwprintw (w,i+1,2,"/%.56s",filestoshow[m+i].name);
	else mvwprintw (w,i+1,2,"%.56s",filestoshow[m+i].name);
      }
      wsetcolor (w,8,0);
...
void showlist(int scr,char s[100],int typeofshow, list_type list, int numar)
{
  int q=0;
  int v;
