...
int load_from_le0_6()
{
  showfiles (14,".","Select an equalizer file to import",0,".");
  if (mpegnumber > -1)
  {
...
int load_from_eqf()
{
  int e=0;
  char eqf[27];
  showfiles (14,".","Select an EQF file to import",0,".");
  if (mpegnumber > -1)
  {
    FILE *f;
...
int load_winamp_preset (char *file, char *preset, char *path)
{
   ...
   if (f!=NULL)
   {
	 ...
	 preset1[j]='\0';
//	 sscanf (temp,"%s",(char*)&preset1);
//         fprintf (stderr,"[%s]",preset1);
