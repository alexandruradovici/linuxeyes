...
//    filelist[prev[nr_prev]].curent=0;
    if (fileexists(name))
    {
      ...
      setcolor (1,0);
      for (unsigned int t=0;t<=nrcols-10;t++)
      {
        mvprintw (4,9+t," ");
        mvprintw (5,9+t," ");
      }
...
void main_options()
{
  ...
  do
  {
    ...
    mvwprintw (w,11,2,"-- Equalizer --");
    // Display
    wsetcolor (w,12,0);
    mvwprintw (w,3,2,"Time display format");
    list_with_key (w,3,22,12,0,4,get_time,time_display,0);
    // Playlist
    check_with_key (w,6,2,"Random next song",get_shuffle,0);
    check_with_key (w,7,2,"Autolocate current song",get_auto_goto,0);
    check_with_key (w,6,32,"Use persistent filter",get_filter,0);
    check_with_key (w,7,32,"Save playlist position on exit",get_save_pos,0);
    wsetcolor (w,12,0);
    mvwprintw (w,9,2,"Auto save playlist to");
    readstring_with_key (w,9,24,40,500,&get_playlist,'\0',NULL,0);
    // Equalizer
    check_with_key (w,12,2,"Use automatic equalization",get_autoeq,0);
    ...
    if (curent==2) key=check_with_key (w,6,2,"Random next song",get_shuffle,1);
    if (curent==3) key=check_with_key (w,7,2,"Autolocate current song",get_auto_goto,1);
    if (curent==4) key=check_with_key (w,6,32,"Use persistent filter",get_filter,1);
    if (curent==5) key=check_with_key (w,7,32,"Save playlist position on exit",get_save_pos,1);
    if (curent==6) key=readstring_with_key (w,9,24,40,255,&get_playlist,'\0',NULL,1);
    // Equalizer
    if (curent==7) key=check_with_key (w,12,2,"Use automatic equalization",get_autoeq,1);
    if (key==1) curent--;
    if (key==2) curent++;
    if (key==3) curent++;
...
// test if n > 16
	...
	mvprintw (7+i,nrcols+1,"  ");
      ...
      if ((radio_on.mode==1) || (radio_off.mode==1)) strcpy (s,"Radio    ");
	...
	if ((filelist[i].min==0) && (filelist[i].sec==0)) tunknown=1;
      ...
      if (save_pos) pos='*';
      if (status_bar==0) mvprintw (nrscr+9,1,"%s | Volume[%d] | Pcm[%d] | Balance[%d] | %c |",s,vol,pcm,balance-50,pos);
      if (status_bar==1) mvprintw (nrscr+9,1,"Next Song: %s",next.text);
      if (status_bar==2) mvprintw (nrscr+9,1,"Filter: %d file(s)",filternr+1);
      if (status_bar==3) mvprintw (nrscr+9,1,"Radio | Start: %s | Stop: %s",radioontext,radioofftext);
