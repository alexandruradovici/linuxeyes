...
int scr=0,scrx=0; //the screen size, normal terminal 16
...
void resettime()
{
    ...
    mvprintw (0,maxl+11,"]");
    setcolor (1,0);
    mvprintw (4,nrcols-7," [00:00] ");
    mvprintw (5,nrcols-22," [--------------------] ");
    n1=-100;
    old_nrcols=nrcols;
}
...
void messages()
{
    ...
    setcolor (1,0);
    if (message.code==XA_MSG_NOTIFY_INPUT_TIMECODE) mvprintw (4,nrcols-6,"[%.2d:%.2d]",message.data.timecode.m,message.data.timecode.s);
    ...
    if (message.code==XA_MSG_NOTIFY_INPUT_POSITION)
    {
      ...
      {
        setcolor (1,0);
        mvprintw (5,nrcols-21,"[--------------------]");
        mvprintw (5,nrcols-21+n,"*");
	n1=n;
      }	
    }
...
void savelist(char *file,int t)
{
  ...
  if (t==1)
  {
    ...
    strcat (tempstring,"/");
    readstring (w,1,11,46,tempstring);
...
int main(int argc,char *argv[])
{
    ...
    strcpy (fs_command[2],"mount /dev/hdd");
    strcpy (fs_command[3],"umount /dev/hdc");
    ...
    add_console_message ("Hello, welcome to LinuxEyes v0.5");
    add_console_message ("--------------------------------");
    strcpy (strtmp,"xmp: starting in: \"");
    strcat (strtmp,path);
    strcat (strtmp,"\"");
    add_console_message (strtmp);
    printf ("Amysoft Linux Eyes v0.5 (C) Eclipse 2001\n\r");
