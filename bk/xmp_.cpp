...
void resettime()
{
  if (screen==0)
  {
    ...
    setcolor (3,1);
    for (int j=0;j<=nrcols+1;j++) mvprintw (0,j," ");
    if (init_text == 1) mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v1.0-pre1 [press h for help]");
...
// test if n > 16
	...
	if (filelist[m+i].sec != 0) mvprintw (7+i,nrcols-5," %.2d:%.2d",filelist[m+i].min,filelist[m+i].sec);
	setcolor (1,0);
...
/*    printf ("%s\n",genres[1]);
    ...
    add_console_message ("Hello, welcome to LinuxEyes v1.0-pre1");
    add_console_message ("--------------------------------");
    strcpy (strtmp,"xmp: starting in: \"");
    strcat (strtmp,path);
    strcat (strtmp,"\"");
    add_console_message (strtmp);
    printf ("Linux Eyes v1.0-pre1 (C) Alexandru Radovici 2002 - 2100\n\r");
    if (argc > 1)
    ...
    setcolor (3,1);
    init_text=1;
    mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v1.0-pre1 [press h for help]");
