...
void resettime()
{
  if (screen==0)
  {
    ...
    setcolor (3,1);
    for (int j=0;j<=nrcols+1;j++) mvprintw (0,j," ");
    if (init_text == 1) mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v0.9 [press h for help]");
...
int saveconfig()
{
  ...
  if (!f.fail())
  {
     ...
     f <<ok_char <<endl;
     f <<screensaver_time <<" " <<scr_autodetect;
...
//    v=getch();
      ...
      list_with_key (w,7,11,16,0,126,my_id3.genre,genres,0);
      readstring_with_key (w,8,11,30,30,&e_comment,'\0',NULL,0);
      ...
      if (curent==6) key=list_with_key (w,7,11,16,0,126,my_id3.genre,genres,1);
      if (curent==7) key=readstring_with_key (w,8,11,30,30,&e_comment,'\0',NULL,1);
...
//	fprintf (stderr,"%s - %s\n",(char*)tempstr,(char*)s2);
	...
	if (more < 0) {k=0; more++;m--; reread=1;}
        if (more > 0) {k=l; more--;if ((m+k) < nr) m++; reread=1;}
	if (k < 0) if (m > 0) { k++;m--; }
        else k++;
        if ((m+k) > nr) k--;
	if (m<0) m=0;
...
void main_options()
{
  ...
  do
  {
    ...
    mvwprintw (w,6,2,"-- Playlist --");
    mvwprintw (w,13,2,"-- Equalizer --");
    mvwprintw (w,16,2,"-- Screen Saver --");
    // Texts
    wsetcolor (w,12,0);
    mvwprintw (w,10,2,"Auto save playlist to");
    mvwprintw (w,11,2,"Replace undisplayable charcaters with");
    mvwprintw (w,17,2,"Start screen saver after");
    mvwprintw (w,17,34,"minutes (0 = Off)");
    // Display
    wsetcolor (w,12,0);
    mvwprintw (w,3,2,"Time display format");
    list_with_key (w,3,22,12,0,4,get_time,time_display,0);
    check_with_key (w,4,2,"Autodetect screen size",get_scr_autodetect,0);
    // Playlist
    check_with_key (w,7,2,"Random next song",get_shuffle,0);
    check_with_key (w,8,2,"Autolocate current song",get_auto_goto,0);
    check_with_key (w,7,32,"Use persistent filter",get_filter,0);
    check_with_key (w,8,32,"Save playlist position on exit",get_save_pos,0);
    readstring_with_key (w,10,24,40,500,&get_playlist,'\0',NULL,0);
    readstring_with_key (w,11,40,1,1,&get_ok_char,'\0',NULL,0);
    // Equalizer
    check_with_key (w,14,2,"Use automatic equalization",get_autoeq,0);
    // Screen Saver
    list_nr_with_key (w,17,27,3,0,60*24,get_screensaver_time,0);
    //Playlist
    if (curent==1) 
    {
    ...
    if (curent==2) key=check_with_key (w,4,2,"Autodetect screen size",get_scr_autodetect,1);
    if (curent==3) key=check_with_key (w,7,2,"Random next song",get_shuffle,1);
    if (curent==4) key=check_with_key (w,8,2,"Autolocate current song",get_auto_goto,1);
    if (curent==5) key=check_with_key (w,7,32,"Use persistent filter",get_filter,1);
    if (curent==6) key=check_with_key (w,8,32,"Save playlist position on exit",get_save_pos,1);
    if (curent==7) key=readstring_with_key (w,10,24,40,255,&get_playlist,'\0',NULL,1);
    if (curent==8) key=readstring_with_key (w,11,40,1,1,&get_ok_char,'\0',NULL,1);
    // Equalizer
    if (curent==9) key=check_with_key (w,14,2,"Use automatic equalization",get_autoeq,1);
    if (curent==10) key=list_nr_with_key (w,17,27,3,0,24*60,get_screensaver_time,1);
    if (key==1) curent--;
    if (key==2) curent++;
    if (key==3) curent++;
...
//  mvwprintw (w,2,1," Playlist Options ");
  ...
  do
  {
    ...
    if (t=='l')
    {
      werase (w);
      wrefresh (w);
      showfiles (nrscr-1,".","Select a playlist to load",0,".");
      if (mpegnumber > -1)
      {
    ...
    if (t=='i')
    {
      ...
      if (format != 0)
      {
        showfiles (nrscr-1,".","Select a playlist to import",0,".");
        if (mpegnumber > -1)
        { 
	  strcpy (playlistfile,mpegfile[0]);
...
// test if n > 16
	...
	for (j=0;j<=maxl;j++) 
	{
	  if (char_ok(filelist[m+i].text[j])) mvprintw (7+i,j+nr_t,"%c",filelist[m+i].text[j]);
	  else mvprintw (7+i,j+nr_t," ");
	}
	if (puncte_puncte==1) mvprintw (7+i,maxl+nr_t+1,"...");*/
	...
	if ((filelist[i].min==0) && (filelist[i].sec==0)) tunknown=1;
      ...
      flags[5]='\0';
      if (status_bar==0) mvprintw (nrscr+9,1,"%s | Volume[%d] | Pcm[%d] | Balance[%d] | %s |",s,vol,pcm,balance-50,flags);
      if (status_bar==1) mvprintw (nrscr+9,1,"Next Song: %s",next.text);
      if (status_bar==2) mvprintw (nrscr+9,1,"Filter: %d file(s)",filternr+1);
      if (status_bar==3) mvprintw (nrscr+9,1,"Alarm | Start: %s | Stop: %s",radioontext,radioofftext);
      if (status_bar==4) mvprintw (nrscr+9,1,"Shut Down At: %s",shutdowntext);
      setcolor (1,1);
...
//      if (v=='l') savelist(playlistfile,1);
	...
	reread=1;
      ...
      if (v=='a')
      {
        showfiles (nrscr-1,".","Select a file to add",0,".");
	...
	if (strcmp(memstring,"")!=0)
	{
      ...
      if (v=='D') 
      { 
        showfiles (nrscr-1,".","Select a directory to add",1,".");
	if (mpegnumber > -1)
	{
...
/*    printf ("%s\n",genres[1]);
    ...
    add_console_message ("Hello, welcome to LinuxEyes v0.9");
    add_console_message ("--------------------------------");
    strcpy (strtmp,"xmp: starting in: \"");
    strcat (strtmp,path);
    strcat (strtmp,"\"");
    add_console_message (strtmp);
    printf ("Linux Eyes v0.9 (C) Alexandru Radovici 2002 - 2100\n\r");
    if (argc > 1)
    ...
    setcolor (3,1);
    init_text=1;
    mvprintw (0,0,"Welcome to LinuxEyes MP3 Player v0.9 [press h for help]");
